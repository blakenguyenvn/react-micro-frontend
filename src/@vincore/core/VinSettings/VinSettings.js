import { useDebounce, usePrevious } from '@vincore/hooks';
import { Controller, useForm } from 'react-hook-form';
import VinLayoutConfigs from '@vincore/layouts/VinLayoutConfigs';
...
function VinSettings(props) {
	...
	const form = watch();
	const { form: formConfigs } = VinLayoutConfigs[form.layout.style];
		...
		if (formChanged) {
			if (settings.layout.style !== newSettings.layout.style) {
				_.set(
					newSettings,
					'layout.config',
					VinLayoutConfigs[newSettings?.layout?.style]?.defaults
				);
			}
		...
		return (
			<Select
				...
				style={{
					...
					color:
						themes[value].palette.type === 'light'
							? '#000000'
							: '#ffffff'
				}}
			>
						...
						>
							...
							>
								<div
									className="w-1/4 h-8"
									style={{
										backgroundColor: val.palette.primary
											.main
								...
								<div
									className="w-1/4 h-8"
									style={{
										backgroundColor: val.palette.secondary
											.main
					...
					render={({ field }) => (
						...
						>
							...
							<RadioGroup
								...
								className={classes.group}
							>
								{Object.entries(VinLayoutConfigs).map(
									([key, layout]) => (
										<FormControlLabel
											key={key}
											value={key}
...
export default memo(VinSettings);
