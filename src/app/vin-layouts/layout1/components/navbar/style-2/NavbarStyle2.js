...
import {
	...
	navbarOpenFolded,
	navbarCloseMobile
} from 'app/store/vin/navbarSlice';
...
const useStyles = makeStyles(theme => ({
	...
	foldedAndClosed: {
		'& $navbarContent': {
			...
			'& .vin-list-item-text, & .arrow-icon, & .item-badge': {
				opacity: 0
			},
			'& .vin-list-subheader .vin-list-subheader-text': {
				opacity: 0
			},
			'& .vin-list-subheader:before': {
				content: '""',
				display: 'block',
				position: 'absolute',
				minWidth: 16,
			...
			>
				<Hidden mdDown>
					<div
						...
						onMouseEnter={() =>
							foldedAndClosed && dispatch(navbarOpenFolded())
						}
						onMouseLeave={() =>
