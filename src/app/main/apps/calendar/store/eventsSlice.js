...
export const getEvents = createAsyncThunk(
	...
	async () => {
		const response = await API.get('/api/calendar-app/events');
...
export const addEvent = createAsyncThunk(
	...
	async (newEvent, { dispatch }) => {
		const response = await API.post('/api/calendar-app/add-event', {
			newEvent
		});
		const data = await response.data;
...
export const updateEvent = createAsyncThunk(
	...
	async (event, { dispatch }) => {
		const response = await API.post('/api/calendar-app/update-event', {
			event
		});
		const data = await response.data;
...
export const removeEvent = createAsyncThunk(
	...
	async (eventId, { dispatch }) => {
		const response = await API.post('/api/calendar-app/remove-event', {
			eventId
		});
		const data = await response.data;
